...
export const SkeletonLoader: React.FC<SkeletonProps> = ({ type, count = 3 }) => {
    ...
    return (
      <div className="space-y-6 w-full animate-pulse">
        ...
        <div className="glass-panel p-6 rounded-3xl border border-white/5 space-y-4">
          <div className="h-6 w-40 bg-slate-800 rounded-md mb-6"></div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {[1, 2, 3].map((i) => (
              <div key={i} className={`h-36 rounded-2xl bg-slate-900/60 border border-white/5 p-4 space-y-3 ${shimmerClass}`}>
        ...
        <div className="glass-panel p-6 rounded-3xl border border-white/5 space-y-4">
          <div className="h-6 w-32 bg-slate-800 rounded-md mb-4"></div>
    ...
    return (
      <div className="space-y-4 w-full animate-pulse">
        <div className="flex items-center gap-3">
          <div className="h-10 flex-1 bg-slate-800 rounded-xl"></div>
          <div className="h-10 w-24 bg-slate-800 rounded-xl"></div>
        </div>
        
        <div className="space-y-3">
          {Array.from({ length: count }).map((_, i) => (
            <div key={i} className={`glass-panel p-5 rounded-2xl border border-white/5 flex flex-col md:flex-row justify-between gap-4 ${shimmerClass}`}>
              <div className="space-y-3 flex-1">
                <div className="flex items-center gap-2">
                  <div className="h-6 w-20 bg-slate-800 rounded-full"></div>
                  <div className="h-4 w-28 bg-slate-800 rounded-md"></div>
                </div>
                <div className="h-5 w-1/2 bg-slate-800 rounded-md"></div>
  ...
  if (type === 'profile') {
    return (
      <div className="glass-panel p-8 rounded-3xl border border-white/5 w-full max-w-2xl mx-auto space-y-6 animate-pulse">
  ...
  return (
    <div className={`h-40 rounded-2xl bg-slate-900 border border-white/5 p-5 animate-pulse ${shimmerClass}`}>
      <div className="h-5 w-1/3 bg-slate-800 rounded-md mb-4"></div>
      <div className="h-8 w-2/3 bg-slate-800 rounded-md mb-2"></div>
